#!/usr/bin/env python3
"""Scrape underground Komplettlösungen and fill walkthrough_* fields by title matching."""
...
from walkthrough_match import (
    ...
    parse_underground_label,
    underground_catalog_id,
)

LIST_BASE = (
    "https://www.maniac-mansion-mania.com/index.php/de/spiele/komplettloesungen/underground.html"
)
LINK_RE = re.compile(
    r'href=["\']([^"\']*komplettloesungen/underground/([^"\']+\.html))["\'][^>]*>([^<]+)',
    re.IGNORECASE,
)
UNDERGROUND_PREFIXES = ("TE-", "MH-", "FM-")


def scrape_underground_links() -> list[tuple[str, str]]:
    session = requests.Session()
    session.headers["User-Agent"] = "mmm-data-scrape-underground-walkthroughs/1.0"
    seen: set[str] = set()
    out: list[tuple[str, str]] = []

    for start in (0, 10, 20, 30):
        url = LIST_BASE if start == 0 else f"{LIST_BASE}?limitstart={start}"
...
def underground_candidates(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    return [r for r in rows if r["catalog_id"].startswith(UNDERGROUND_PREFIXES)]
...
def match_links_to_catalog(
    ...
    cands = underground_candidates(rows)
    ...
    for label, url in links:
        hint = parse_underground_label(label)
        cid = underground_catalog_id(hint)
        if cid and cid in by_id and cid not in used_ids:
            url_by_id[cid] = url
            used_ids.add(cid)
            continue
        if hint.get("kind") == "title":
            cid = fuzzy_match_catalog_id(
                hint.get("title", label),
                [(i, t) for i, t in cand_titles if i not in used_ids],
                min_ratio=0.68,
            )
            if cid:
                url_by_id[cid] = url
...
def apply_underground(
    ...
    *,
    ...
    links = scrape_underground_links()
    print(f"scraped {len(links)} underground walkthrough links")
    ...
    if unmatched_catalog:
        print("catalog underground still without URL:", ", ".join(unmatched_catalog))
    ...
    for i, row in enumerate(pending, start=1):
        ...
        print(f"[{i}/{total}] {cid} {row['title'][:55]}")
        try:
