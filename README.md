...
**CI:** on every push and pull request to `main`, [GitHub Actions](.github/workflows/catalog-ci.yml) runs the same `build_catalog.py` step on Ubuntu and fails the job if the working tree would change (for example CSV updated but `data/catalog.jsonl` not regenerated and committed).
